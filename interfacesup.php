  ...
  <div class="collapse navbar-collapse" id="navbarNavDropdown">
    ...
    <ul class="navbar-nav"> 
      <li class="nav-item active">
        ...
        <a style="margin-left:15px;" class="nav-link bg-primary text-white" href="?do=manage"><i class="fa fa-home fa-fw text-white " aria-hidden="true"></i>&nbsp;Accueil</a>
      </li>
 <li class="nav-item">
...
<table class="table table-bordered">
 ...
 ?>
    ...
    <a role="button" class="btn btn-primary form-control" href="?do=add" ><i class="fa fa-plus">Primary</i></a>
 <?php
...
//echo 'Il y a '.$jours_ouvres.' jours ouvrés entre le '.date('d/m/Y', $date_depart).' et le '.date('d/m/Y', $date_fin);
    ...
    $stmt = $conn->prepare(" SELECT `CIN`, `ID`, `DureConge`, `DateDebut`, `DateDemande`, `Datefin`, `Observation`,`Etat` from conge where visaCSRH=? AND visaDirecteur='en attente' "); 
...
<div class="container" style="width:800px">
    <div class="card">
        <h3 class="text-center"> Demandes en attente de validation </h3>
<table class="table table-bordered">
  <thead class="thead-dark">
 ...
 foreach($rows as $row)
 {
   ...
   <tr>  
       ...
       <td><?php echo $row['Observation'] ;?></td>  
        <td><?php echo $row['Etat'] ;?></td>
      <td>
          <a href="?do=Oui&id=<?php echo $row['ID']; ?>&CIN=<?php echo $row['CIN'] ;?>"><i class="fa fa-edit text-success"> accepter</i></a>
...
<?php     
    } else if($_GET['do']=='Oui')
      {
        ...
        $req1 = $conn->prepare("UPDATE `conge` SET `visaDirecteur`=1 WHERE `CIN`='$cin' AND `ID`='$id'");
        // $req2 = $conn->prepare("UPDATE `conge` SET `Etat`='acceptée' WHERE `CIN`='$cin' AND `ID`='$id'");
        $req1->execute();
       // $req2->execute();
     ...
     $req1 = $conn->prepare("UPDATE `conge` SET `visaDirecteur`=0 WHERE `CIN`='$cin' AND `id`='$id'");
     $req1->execute();
...
$req = $conn->prepare(" update conge set DureConge=?,DateDebut=? , DateFin=? , Observation=? where id=?");
          $req->execute(array($duree, $deb ,$fin ,$obsrv, $id));
             echo "<h3 class='text-success'>Modification réussite </h3>";
 ...
 }else if($_GET['do']=='deleteDem')
{
    ...
    $req1 = $conn->prepare("delete from conge where id=? ");
     ...
     header("refresh:1;url=?do=MesDemandes");
     echo "<h3 class='text-success'>la demande est bien supprimée</h3>";
